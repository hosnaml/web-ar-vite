...
import Podme2 from "../assets/Podme2.jpg";

const Podme2ImagePlane = (props) => {
  ...
  useFrame(({ camera }) => {
    ...
    if (!initialPositionSet.current) {
      ...
      console.log(
        "Podme2ImagePlane - Initial position set with X offset:",
  ...
  useEffect(() => {
    const componentId = "Podme2ImagePlane";
    console.log("Starting Podme2 texture loading...");
    console.log("Loading image from path:", Podme2);
    ...
    textureLoader.load(
      Podme2,
      (loadedTexture) => {
        console.log("Podme2 texture loaded successfully");
        ...
        if (materialRef.current) {
          ...
          console.log("Updated Podme2 material with texture");
        }
      },
      // Progress callback
      (xhr) => {
        console.log(
          `Podme2 texture loading: ${(xhr.loaded / xhr.total) * 100}% loaded`
        );
      },
      // Error callback
      (error) => {
        console.error("Error loading Podme2 texture:", error);
  ...
  const createCanvasTexture = () => {
    ...
    gradient.addColorStop(0, "#db4437"); // Different colors for Podme2
    ...
    ctx.fillText("PODME 2", canvas.width / 2, canvas.height / 2);
    ctx.font = "32px Arial";
    ...
    setLoaded(true);
    console.log("Podme2 canvas texture created successfully");

    // Add canvas to DOM for debugging if needed
    if (DEBUG) {
      canvas.style.position = "absolute";
      canvas.style.bottom = "10px";
      canvas.style.right = "190px"; // Different position from other debug elements
      canvas.style.width = "80px";
      canvas.style.height = "140px";
      canvas.style.zIndex = "1000";
      canvas.style.border = "2px solid green"; // Different color for identification
...
export default Podme2ImagePlane;
